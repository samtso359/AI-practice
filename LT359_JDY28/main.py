...
if __name__ == "__main__":
    ...
    # BFS
    graph = _get_test_graph_admissible()
    (expansion_count, cost, path) = _graph_search(graph, 1, 5, mp.add_to_queue_BFS, mp.is_queue_empty_BFS, mp.pop_front_BFS, _heuristic_zero)
    print "BFS path: " + str(path) + ", cost: " + str(cost) + ", #expansions: "+ str(expansion_count)
...
print "Final state after hill-climbing: " + str(new_state) + ", conflicting pairs: " + str(mp.compute_attacking_pairs(new_state))
    #
    # # Get a fully solved state for a given n
print "A valid solution: " + str(mp.n_queens(n,mp.get_random_state, mp.compute_attacking_pairs,mp.hill_desending_n_queens))
    #
    #
